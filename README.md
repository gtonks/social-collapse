...
Figure 1: This was created using Desmos without writing code.

Figure 2: `dhanalysis/Social_Collapse_linear_consumer_birth.m` (MATLAB script). Note: Optimization Toolbox must be installed.

Figures 3, 4: These were created using MATCONT without writing code.

Figure 5: `dhanalysis/Social_Collapse_nonlinear_consumer_birth.m` (MATLAB script). Note: Optimization Toolbox must be installed.

Figure 6: `python -m socialcollapse.eulermaruyama.mu_var_linear_dH`

Figure 7: Compute data using `python -m socialcollapse.eulermaruyama.final_search_over_a_mu_var_linear_dH`. Figure comes from `eulermaruyama/Final_Search_a_Analysis.ipynb`. See `python -m socialcollapse.eulermaruyama.plot_prob_mesh_over_a` for a 3D visualization.

Figure 8: Compute data using `python -m socialcollapse.eulermaruyama.final_search_over_b_mu_var_linear_dH`. Figure comes from `eulermaruyama/Final_Search_b_Analysis.ipynb`. See `python -m socialcollapse.eulermaruyama.plot_prob_mesh_over_b` for a 3D visualization.

Figure 9: Compute data using `python -m socialcollapse.eulermaruyama.final_search_over_b_mu_var_linear_dH`. Figure comes from `eulermaruyama/Final_Search_b_Analysis.ipynb`.
